...
static void handle_sigfpe(int num, siginfo_t *info, void *_context)
{
    ...
    mcontext->__gregs[REG_PC] += 4; // RISC-V 每条指令通常 4 字节
#else
#warning "Unsupported architecture: signal handling might not work"
#endif
	return;
}
...
typedef struct {
    double f[32];   // 保存 f0 ~ f31
    unsigned int fcsr; // 浮点控制状态寄存器
} riscv_fpu_state_t;

#define save_fpu_state(state) do {                             \
    __asm__ volatile (                                         \
        "fsd f0, 0(%0)\n\t"  "fsd f1, 8(%0)\n\t"               \
        "fsd f2, 16(%0)\n\t" "fsd f3, 24(%0)\n\t"              \
        "fsd f4, 32(%0)\n\t" "fsd f5, 40(%0)\n\t"              \
...
// #define fxsave(addr) __asm __volatile("fxsave %0" : "=m"(*(addr)))
	...
	#define fxsave(addr) __asm __volatile("fxsave %0" : "=m"(*(addr)))
#elif __riscv
	#define fxsave(addr) save_fpu_state((riscv_fpu_state_t *)(addr))
#endif
