...
use crate::{
    arch::{boot::DEVICE_TREE, device::io_port::PortRead, irq::IrqLine},
    bus::pci::capability,
    io::IoMem,
...
impl PLIC {
    ...
    pub fn chaim(&self, hart_id: usize, is_s_mode: bool) -> Option<u32> {
        let context_id = Self::get_context_id(hart_id, is_s_mode);
...
pub fn handle_external_interrupt(f: &mut TrapFrame) {
    let plic = PLIC.get().unwrap();
    let hart_id = 0;
    let interrupt = plic.chaim(hart_id, true);
    if let Some(interrupt) = interrupt {
        let iqr_line = unsafe { IrqLine::acquire(interrupt as u8) };
        for callback in iqr_line.callback_list().iter() {
            callback.call(f);
        }
        plic.complete(hart_id, true, interrupt as u8);
    }
}
